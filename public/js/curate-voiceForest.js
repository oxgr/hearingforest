...
const entries = await requestEntries( 'all' );
showEntries( entries, model.html.entries );

async function requestEntries( filter ) {

    // Inital GET HTTP request
    const response = await fetch( origin + '/voiceForest/json/' + filter );
...
async function showEntries( entries, parent ) {
    ...
    entries.forEach( ( entry, index ) => {
        ...
        <div class="entryBody audioPlayerContainer">
            <audio
                controls
                class="audioPlayer"
                id="${entry.id}"
                preload="none"
                src="${model.origin}/voiceForest/audio/${entry.id}">
        ...
        // const audioPlayer = entryDiv.querySelector( '.audioPlayer');
        
        // console.log( {style: audioPlayer.style} )
        // audioPlayer.style.width = '50%';
        // audioPlayer.style.border = '2px solid white';
        // audioPlayer.style.backgroundColor = '#101010';
...
function onFooterButton( event ) {
    ...
    fetch( model.origin + '/voiceForest/curate', {
        method: 'POST', // or 'PUT',
        mode: 'cors',
        headers: {
...
async function playEntryAudio( event ) {

    console.debug( 'playEntryAudio()' );
    console.debug( event );
